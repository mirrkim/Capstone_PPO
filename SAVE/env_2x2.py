...
class DroneEnv:
    ...
    def _rebuild_obs_cache(self):
        obs_arr = np.array(self.obstacles, dtype=np.float32)
        r_arr   = np.array(self.obs_radii,  dtype=np.float32)
        valid   = r_arr > 0
        self._valid_obs = obs_arr[valid]
        self._valid_r   = r_arr[valid]
    ...
    def _calc_soft_penalty(self, pos, min_dist_obs,
                           obs_margin=70.0, wall_margin=70.0,
                           near_crash_margin=20.0,
                           in_signal=False):
        """
        장애물·벽 근접 패널티.
        in_signal=True 이면 계수를 2배 적용 →
        신호 추적 보상이 패널티를 압도하지 못하게 차단.
        """
        ...
        wall_dist = float(min(pos[0], pos[1], 700 - pos[0], 700 - pos[1]))
        if wall_dist < wall_margin:
            soft -= ((wall_margin - wall_dist) / wall_margin) ** 2 * 130.0 * scale
    ...
    def _generate_world(self):
        ...
        while True:
            ...
            if px + py > 700:
                ...
                self._rebuild_obs_cache()
                if len(self._valid_obs) == 0 or \
                   np.min(np.linalg.norm(candidate_pos - self._valid_obs, axis=1)
                          - self._valid_r) >= 30.0:
    ...
    def get_state(self):
        ...
        if len(self._valid_obs) > 0:
            rel_pos  = (self._valid_obs - self.drone_pos) / 700
            dists    = np.linalg.norm(rel_pos, axis=1) - self._valid_r / 700
    ...
    def step(self, action):
        ...
        soft_penalty = self._calc_soft_penalty(
            new_pos, min_dist_to_obs,
            in_signal=(curr_bpsk > 0.0)
        )
        ...
        if curr_bpsk > 0.0:
            # ┌──────────────────────────────────────────────────────────┐
            # │ [수정 1] safety_factor                                   │
            # │  min_dist < obs_margin(70) 이면 신호 보상이 선형 감쇠     │
            # │  min_dist = 0  → 신호 보상 0 (장애물 충돌 직전엔 0원)    │
            # │  min_dist ≥ 70 → 신호 보상 100% (안전 거리 밖은 그대로)  │
            # └──────────────────────────────────────────────────────────┘
            safety_factor = float(np.clip(min_dist_to_obs / obs_margin, 0.0, 1.0))

            rssi_diff_rew    = bpsk_diff * 300.0
            proximity_reward = curr_bpsk * 80.0

            # [수정 2] path_blocked 시 rssi_diff도 함께 감쇠
            #          기존: proximity만 0.2배, rssi_diff는 그대로 →
            #          돌진하면서 RSSI 빠르게 오르면 보상 폭등
            if self._path_blocked(new_pos, self.bpsk_pos, margin=20.0):
                proximity_reward *= 0.2
                rssi_diff_rew    *= 0.3

            # [수정 3] 신호 보상 전체에 safety_factor 적용
            signal_reward = (rssi_diff_rew + proximity_reward) * safety_factor

            reward = step_penalty + signal_reward + soft_penalty + info_reward
            ...
            if self.chosen_target in candidates:
                ...
                if len(candidates) > 1:
                    ...
                    dist_costs     = np.linalg.norm(
                        target_centers - self.drone_pos, axis=1)
