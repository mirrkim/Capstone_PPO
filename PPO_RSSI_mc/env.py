...
COVERAGE_N = 70
COVERAGE_REWARD_SCALE = 2.0   # 색칠 보상 스케일(새 칸 × belief지수가중 × 이 값)
...
class DroneEnv:
    ...
    def get_amc_rssi(self, pos):
        ...
        # AMC 분류 실행
        # BPSK가 감지되면(범위 안이면) QAM보다 우선해서 BPSK로 분류한다.
        # (목표는 BPSK 신호원이므로, QAM이 더 강해도 BPSK가 잡히면 BPSK를 따라간다)
        if raw_bpsk > 0.0:
    ...
    def step(self, action):
        ...
        if curr_bpsk > 0.0: 
            ...
            #   - 11% → 0.85, 13% → 1.0(기준), 20% → 1.75, 30% → 3.9, 40% → 8.7
            weight = np.exp(8.0 * (belief_before - 0.13))   # (9,)
